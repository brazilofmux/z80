...
- **A SORT for CP/M.** `cpm/sort/sort.asm` is an external sort in Z80 assembly, the SORT step CP/M 2.2 never shipped: DFSORT-style decks (`SORT FIELDS=(pos,len,CH,A,...)`, CI folds case, D descends, fields past a record read as spaces), stable, records held whole in the 40K above the program and merged sixteen runs at a time across B: and C:. `make sort` builds `SORT.COM` with Macro Assembler AS; it asks three console lines (SORTIN, SORTOUT, SYSIN deck), so a `--script` job stream drives it like JCL. It sorts a 55,000-line file in under a second on the JIT; `tests/sort.sh` checks it against a Python reference.
- **Loop idioms in closed form.** 8080-style byte copy and fill loops (`LD A,(HL); LD (DE),A; INC HL; INC DE; DEC r; JP NZ` — 8080 code has no `LDIR`) run as one helper call each, and WordStar's filtered text-copy loop (mask, bound check, sentinel check, copy, count) does too, with its three exits and their flags reproduced exactly — the first piece of the per-application road, matched by shape with the constants as parameters.
- **Countdown loops in closed form.** `DEC r; JP NZ,self`, the `JR` form and `DJNZ $` touch nothing but one register, so the translator emits their exit state and adds the skipped instructions to the count — lockstep-exact. 1980s software is full of them: they are delay loops calibrated for 4 MHz, and WordStar's 256-spin timing tick was 22% of every instruction it executed during a global search-and-replace.
...
To boot the real thing: `tools/mkdsk -f hd work.dsk your/*.COM` makes an 8 MB image (or `-f fd` a 400K floppy), `./z80-monster -j -K -A work.dsk` boots CP/M 2.2 on it (up to `-P`), `^]` quits, `tools/mkdsk -x work.dsk -o out/` gets files back. `make system` rebuilds `cpm/cpm22/system.bin` from the DRI sources and `cpm/cpm22/bios.asm` if you have run `tools/get-asl.sh`.

For a single program without an image, the directory containing the `.COM` file becomes drive A:. Console I/O is raw termios with buffered output; `-K` paints a Kaypro screen instead (`Z80_KEYS=wordstar` maps the arrow keys the way WordStar likes, `--no-hud` drops the BIPS line). WordStar 3.0's INSTALL predates the Kaypro; install it for the ADM-3A and run `tools/wskaypro.sh WS.COM` to add the Kaypro '84 inverse-video and erase-to-end-of-line strings, which is what makes its menus and status line look like the real machine's. `--script FILE` drives a program headlessly from a keystroke script with `@wait-idle` / `@dump` (see `kaypro/kaypro_kbd.h`); `make test-apps` runs WordStar and Zork that way, under the JIT or `-V` lockstep.
...
1. **It boots CP/M now — but only ours.** `./z80-monster -A disk.dsk` runs Digital Research's real CP/M 2.2 CCP and BDOS (assembled byte-exact from the released sources) on a BIOS of ours that reaches the host through a dozen I/O ports: you get the `A>` prompt, `DIR`, `PIP`, `SUBMIT`, and every FCB/extent/user-area semantic DRI wrote. What you don't get is a Kaypro's own boot ROM and floppy controller; disks are raw `.DSK` images in two formats `tools/mkdsk` and cpmtools understand. The older path — run one `.COM` against a C shim mapping a host directory to A: — is still there and is what the benchmarks use.
2. **You can't experience 8,000× at a prompt anyway.** A prompt waits for a human at any speed. The speed shows in compute: MS-COBOL 4.65 compiles and links a program before the terminal finishes repainting. The Kaypro terminal exists now (`-K`: an 80×25 cell buffer, the Kaypro '84 escape set, a diffing painter on your terminal, a BIPS line under it), and WordStar 3.00, dBASE II and Turbo Pascal 3 run on it — but through a BDOS shim, with no interrupts, and the shim's fidelity is exactly what those three programs have needed so far.

...
/* Z80_NO_FOLD=1 leaves the countdown / copy / fill / filtered-copy loops
 * translated instruction by instruction, for A/B measurement. */
static int s_fold_loops = -1;
 ...
 * after the backward JP NZ. Returns 12 on a match. (AArch64 matcher
 * verbatim.) */
...
/* Args: (cpu, spec, head, exit) = rdi, esi, edx, ecx. EDX/ECX are the
 * pinned F/A; the wrapper saves and restores them, and both are reloaded
 * from the context afterwards anyway. */
static void emit_filtercopy(emit_t *e, uint32_t spec, uint16_t head, uint16_t exit) {
    ...
    emit_mov_m8_r8  (e, R_CPU, X64_NOREG, OFF_F,  R_F);
    emit_mov_r32_imm32(e, T1, spec);
    emit_mov_r32_imm32(e, R_F, head);
    emit_mov_r32_imm32(e, R_A, exit);
    emit_call_helper(e, (void *)(uintptr_t)z80_jit_loop_filtercopy);
    emit_movzx_r32_m16(e, R_BC, R_CPU, X64_NOREG, OFF_BC);
    emit_movzx_r32_m16(e, R_DE, R_CPU, X64_NOREG, OFF_DE);
    emit_movzx_r32_m16(e, R_HL, R_CPU, X64_NOREG, OFF_HL);
    emit_movzx_r32_m8 (e, R_A,  R_CPU, X64_NOREG, OFF_A);
...
uint8_t *dbt_translate_block(z80_dbt_t *dbt, uint16_t guest_pc) {
    ...
    if (s_fold_loops < 0)
        s_fold_loops = getenv("Z80_NO_FOLD") == NULL;
    ...
    for (uint32_t i = 0; i < n_ops; i++) {
        ...
        int fuse = s_fold_loops ? countdown_kind(decs, pc_afters, i, n_ops) : FUSE_NONE;
        if (fuse != FUSE_NONE) {
            dbt->loops_folded++;

...
int z80_step(z80_cpu_t *cpu) {
    ...
    case Z80_OP_JP_NN:
        ...
        if (!cpu->defer_traps && cpm_traps_enabled && cpu->pc == CPM_BDOS_ENTRY) {
            int cont = cpm_bdos_dispatch(cpu);
            if (!cont) return 1;
            cpu->pc = cpu->mem[cpu->sp] | (cpu->mem[(cpu->sp + 1) & 0xFFFF] << 8);
            cpu->sp = (cpu->sp + 2) & 0xFFFF;
        } else if (!cpu->defer_traps && cpm_traps_enabled && cpu->pc >= CPM_BIOS_BASE && cpu->pc < CPM_BIOS_BASE + 0x80) {
    ...
    case Z80_OP_CALL_NN:
        ...
        if (!cpu->defer_traps && cpm_traps_enabled && (cpu->pc == CPM_BDOS_ENTRY || cpu->pc == CPM_WBOOT_ENTRY)) {
            int cont = cpm_bdos_dispatch(cpu);
            if (!cont) {
         ...
         */
        if (!cpu->defer_traps && cpm_traps_enabled && cpu->pc >= CPM_BIOS_BASE &&
